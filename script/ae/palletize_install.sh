...
CreatePackage(){
	...
	if [ $1 = "pack-palletize" ]; then
		ColorEcho "Generating palletize package..."
		cd "$PACKAGE_SOURCE_DIR"palletize/RC
		tar -cf palletize.tar palletize
		tar -cf "$PACKAGE_DEST_DIR"palletize/update.tar -C "$PACKAGE_SOURCE_DIR"palletize .
		cd "$PACKAGE_DEST_DIR"palletize
...
CreateDIR
#创建弧焊包
CreatePackage pack-palletize $1
#生成结束
GenerateFinish
